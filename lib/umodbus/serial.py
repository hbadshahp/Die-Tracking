...
class Serial(CommonModbusFunctions):
    ...
    def __init__(self,
        ...
        if ctrl_pin is not None:
            ...
            #print(f"Control pin object created: {self._ctrlPin}")
        else:
            self._ctrlPin = None

        # timing of 1 character in microseconds (us)
        self._t1char = (1000000 * (data_bits + stop_bits + 2)) // baudrate
        print(f"timing object of 1 character in miroseconds: {self._t1char}")
        ...
        if baudrate <= 19200:
            ...
            print(f"inter-frame delay object of in miroseconds: {self._inter_frame_delay}")
        else:
    ...
    def _send(self, modbus_pdu: bytes, slave_addr: int) -> None:
        ...
        '''if self._has_uart_flush:
            print("_has_uart_flush")
            self._uart.flush()
            time.sleep_us(self._t1char)
        else:
            print("No..........._has_uart_flush")
            sleep_time_us = (
        ...
        sleep_time_us = (self._t1char * len(modbus_adu) -  time.ticks_diff(send_finish_time, send_start_time))
        time.sleep_us(sleep_time_us)
        if self._ctrlPin:
    ...
    def _send_receive(self,
        ...
        self._uart.read()
        #print(f"flush the Rx FIFO buffer: {self._uart.read()}")
        

        self._send(modbus_pdu=modbus_pdu, slave_addr=slave_addr)
        
        #print(f"control pin condition befor read: {self._ctrlPin.value()}")
        #response=self._uart_read()
        #print(f"response from slave inside _send_recive: {response}")
