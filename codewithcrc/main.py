...
def main():
    while True:
        try:
            ...
            same_tag_counter = 0
            no_tag_counter = 0
            crc_error_counter = 0  # Track CRC errors
            ...
            while True:
                ...
                if rev is not None:
                    # CRC verification passed, process the tag data
                    curr_card = str("".join(rev[7:19]))
                    crc_error_counter = 0  # Reset CRC error counter
                    ...
                    # Could be no tag or CRC error
                    no_tag_counter += 1
                    same_tag_counter = 0
                    crc_error_counter += 1
                    
                    # If we have too many CRC errors, there might be communication issues
                    if crc_error_counter > 10:
                        print("Warning: Multiple CRC errors detected. Check UHF communication.")
                        crc_error_counter = 0  # Reset to avoid continuous warnings
                ...
                # Publish the data regardless of tag status
                card_id_json = {
                    "card": card_pub,
                    "client_id": client_id,
                    "die_height": msg_str
